...
# ============================================================================
# test_stage4.sh — Integration tests for Stage 4 orchestrator
# ============================================================================
#
# Tests stage4-prd-generation.sh with a mock Claude CLI that writes
# canned PRD output files.
#
# Test cases:
#   IT-S4-001: Creates prd_output_tv-accepted/prd.md for accepted plans
#   IT-S4-002: stage4_summary.json has correct counts
#   IT-S4-003: REJECTED Stage 3 plans are skipped
#   IT-S4-004: Quality score below 85% → finding REJECTED
# ============================================================================
...
# ---------------------------------------------------------------------------
# IT-S4-001: Creates PRD output for accepted plans
# ---------------------------------------------------------------------------

test_creates_prd_output() {
    echo "IT-S4-001: Creates prd_output_tv-accepted/prd.md for accepted plans"
    ...
    PATH="$mock_dir:$PATH" "$SCRIPT_DIR/stage4-prd-generation.sh" \
...
# ---------------------------------------------------------------------------
# IT-S4-002: Summary counts
# ---------------------------------------------------------------------------

test_summary_counts() {
    echo "IT-S4-002: stage4_summary.json has correct counts"
    ...
    if [[ -f "$tmpdir/output/stage4_summary.json" ]]; then
        local stage
        stage=$(python3 -c "import json; print(json.load(open('$tmpdir/output/stage4_summary.json'))['stage'])")
        ...
        fail_test "stage4_summary.json" "File not found"
    fi
...
test_rejected_skipped() {
    echo "IT-S4-003: REJECTED Stage 3 plans are skipped"
    ...
    if [[ -f "$tmpdir/output/stage4_summary.json" ]]; then
        local processed
        processed=$(python3 -c "import json; print(json.load(open('$tmpdir/output/stage4_summary.json'))['findings_processed'])")
...
# ---------------------------------------------------------------------------
# IT-S4-004: Quality score below 85% → REJECTED
# ---------------------------------------------------------------------------

test_low_score_rejected() {
    echo "IT-S4-004: Quality score below 85% → finding REJECTED"
    ...
    if [[ -f "$tmpdir/output/stage4_summary.json" ]]; then
        local rejected
        rejected=$(python3 -c "import json; print(json.load(open('$tmpdir/output/stage4_summary.json')).get('rejected', 0))")
