...
# ============================================================================
# test_stage14.sh — Integration tests for Stage 14 pull request creation
# ============================================================================
#
# Tests stage14-pull-request.sh with mock gh CLI and mock make.
#
# Test cases:
...
EOF
    ...
    # Verification result
    cat > "$run_dir/verification_stage11_tv-pr.json" <<'EOF'
{
...
test_pr_created() {
    ...
    local exit_code=0
    PATH="$mock_dir:$PATH" "$SCRIPT_DIR/stage14-pull-request.sh" \
        --run-dir "$run_dir" \
    ...
    if [[ -f "$mock_dir/.gh_log" ]] && grep -q "pr create" "$mock_dir/.gh_log"; then
        # Check stage14_summary.json
        if [[ -f "$output_dir/stage14_summary.json" ]]; then
            local pr_url
            pr_url=$(python3 -c "import json; print(json.load(open('$output_dir/stage14_summary.json'))['pr_url'])" 2>/dev/null || echo "")
...
test_sync_staging() {
    ...
    if [[ -f "$mock_dir/.make_log" ]] && grep -q "sync-staging" "$mock_dir/.make_log"; then
        ...
        if [[ -f "$output_dir/stage14_summary.json" ]]; then
            local sync_result
            sync_result=$(python3 -c "import json; print(json.load(open('$output_dir/stage14_summary.json')).get('sync_staging_result', ''))" 2>/dev/null || echo "")
...
test_pr_description_metadata() {
    ...
    if [[ -f "$mock_dir/.pr_body" ]]; then
        ...
        if [[ -f "$output_dir/stage14_summary.json" ]]; then
            pass_test "Stage 14 summary produced (PR body capture unavailable)"
        else
...
echo "=== Stage 14 Integration Tests ==="
echo ""
